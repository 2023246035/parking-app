...
def payment_field(label: str, placeholder: str, icon: str) -> rx.Component:
    return rx.el.div(
        rx.el.label(label, class_name="block text-sm font-medium text-gray-700 mb-1"),
        rx.el.div(
            rx.el.input(
                placeholder=placeholder,
                class_name="w-full pl-10 pr-4 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-sky-200 focus:border-sky-500 outline-none transition-all",
            ),
            rx.icon(
                icon,
            ...
            rx.cond(
                ...
                rx.el.div(
                    rx.el.div(
                        ...
                        rx.el.div(
                            payment_field("Expiry Date", "MM/YY", "calendar"),
                            payment_field("CVC", "123", "lock"),
                            class_name="grid grid-cols-2 gap-4",
                        ),
                        payment_field("Cardholder Name", "Full Name on Card", "user"),
                        rx.cond(
